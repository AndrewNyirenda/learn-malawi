...
/>
    ...
    <div className="filter-bar" style={{ margin: '0 auto 1.5rem', maxWidth: 'var(--container-max)', width: 'calc(100% - 2*var(--container-pad))' }}>
      ...
      <div className="filter-group">
        ...
        <select className="filter-select" value={subjectFilter} onChange={e => { setSubjectFilter(e.target.value); setCurrentPage(1); }}>
          <option value="all">All Subjects</option>
          {(subjects || []).map((subject, i) => <option key={i} value={subject}>{subject}</option>)}
        </select>
      </div>
      <div className="filter-group">
        <label className="filter-label">Class</label>
        <select className="filter-select" value={classFilter} onChange={e => { setClassFilter(e.target.value); setCurrentPage(1); }}>
          <option value="all">All Classes</option>
          {getSortedClasses().map((cls, i) => <option key={i} value={cls}>{cls}</option>)}
        </select>
      </div>
      {(subjectFilter !== 'all' || classFilter !== 'all') && (
        <button className="filter-reset-btn" onClick={() => { setSubjectFilter('all'); setClassFilter('all'); setCurrentPage(1); }}>
