// src/components/admin-componenents/TutorialsAddModal.jsx
...
const TutorialsAddModal = ({ onClose, onSave, tutorialToCopy = null }) => {
  const { createTutorial, loading, error, clearError } = useTutorials();
  const { user } = useAuth();
  ...
  // Initialize form with tutorial to copy if provided
  useEffect(() => {
    if (tutorialToCopy) {
      setFormData({
        title: tutorialToCopy.title,
        description: tutorialToCopy.description || '',
        level: tutorialToCopy.level,
        subject: tutorialToCopy.subject,
        class: tutorialToCopy.class,
        videoUrl: tutorialToCopy.videoUrl || ''
      });
      
      if (tutorialToCopy.videoUrl) {
        generateVideoPreview(tutorialToCopy.videoUrl);
      }
    }
  }, [tutorialToCopy]);
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h3>
            <FaPlay /> {tutorialToCopy ? 'Duplicate Tutorial' : 'Add New Tutorial'}
          </h3>
        ...
        {submitSuccess ? (
          <div className="tutorial-success">
            ...
            <h4>Tutorial Created Successfully!</h4>
            <p>The tutorial has been added and is now available for students.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="user-form">
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                disabled={loading}
              >
                {loading ? 'Creating...' : tutorialToCopy ? 'Duplicate Tutorial' : 'Create Tutorial'}
              </button>
            </div>
          </form>
        )}
...
export default TutorialsAddModal;